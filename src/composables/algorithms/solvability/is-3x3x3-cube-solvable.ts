...
export function is3x3x3CubeSolvable(): boolean {
  ...
  // 3. Determine the standard colors based on the center stickers
  const faceColors: Partial<Record<THexahedronFaces, TCubeFaceColor>> = {};
  for (const face of HexahedronFaces) {
    const faceValues = logicalValues[face];
    if (!faceValues || faceValues[4] === null || faceValues[4] === undefined) return false;
    faceColors[face] = faceValues[4] as TCubeFaceColor;
  }

  const centerColors = Object.values(faceColors) as TCubeFaceColor[];
  if (centerColors.length !== HexahedronFaces.length) return false;
  if (new Set(centerColors).size !== HexahedronFaces.length) return false; // Centers must be unique

  // Validate full color distribution: exactly 9 stickers for each center color
  ...
  for (const face of HexahedronFaces) {
    ...
    for (const color of faceValues) {
      ...
      if (!centerColors.includes(color as TCubeFaceColor)) return false;
  ...
  for (let pos = 0; pos < CORNER_MAPPING_3X3.length; pos++) {
    const corner = cornersByPosition.get(CORNER_MAPPING_3X3[pos].pieceIdx);
  ...
  for (let pos = 0; pos < EDGE_MAPPING_3X3.length; pos++) {
    const edge = edgesByPosition.get(EDGE_MAPPING_3X3[pos].pieceIdx);
  ...
  for (let i = 0; i < EDGE_MAPPING_3X3.length; i++) {
    ...
    if (value === undefined) return false;
    if (seenEdges.has(value)) return false; // Duplicate edge piece -> invalid state
    seenEdges.add(value);
